...
def test_stock_spot():
    ...
    if not spot:
        ...
        print(f"✓ 测试通过: 成功获取实时行情")
        print(f"  数据项数: {len(spot)}")
        print(f"  前5项数据:")
        for i, (k, v) in enumerate(list(spot.items())[:5]):
            print(f"    {k}: {v}")
        return True
...
def main():
    ...
    results.append(("获取实时行情", test_stock_spot()))
